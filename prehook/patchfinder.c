...
uintptr_t* find_dt_load(uintptr_t* baseaddr, uint32_t isize){ /* 47ffe */
    uintptr_t* add;
    uint16_t search[4];
    /*
     add  r2, sp, #0x4  : 01AA
    ...
    add = memmem(baseaddr, isize, &search, 4 * sizeof(uint16_t));
    if(!add){
        return 0;
    }
...
uintptr_t* find_mount_and_boot_system(uintptr_t* baseaddr, uint32_t isize){
    ...
    search1[4] = 0x74; /* 't' */
    search1[5] = 0x00; /* '.' */
    boot = memmem(baseaddr, isize, &search1, 6 * sizeof(uint8_t));
    if(!boot){
        return 0;
    }
    
    search2[0] = 0x40060001;
    search2[1] = boot;
    ptr = memmem(baseaddr, isize, &search2, 2 * sizeof(uint32_t));
    if(!ptr){
        return 0;
    }
    return ptr;
...
uintptr_t* find_hook_point(uintptr_t* baseaddr, uint32_t isize){
    ...
    search[4] = 0x03;
    search[5] = 0x20;
    ptr = memmem(baseaddr, isize, &search, 6 * sizeof(uint8_t));
    if(!ptr){
        return 0;
    }
    return ptr;
